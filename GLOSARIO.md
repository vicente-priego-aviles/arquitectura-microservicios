...
| — | Saga | Patrón para mantener la consistencia de una transacción de negocio que abarca varios microservicios (y, por tanto, varias bases de datos autónomas) sin una transacción distribuida clásica (2PC): la transacción se parte en una secuencia de pasos locales, cada uno con su propia acción de compensación que deshace su efecto si un paso posterior falla; sin traducción española de uso consolidado |
| Coreografía | Choreography | Forma de implementar una Saga sin coordinador central: cada microservicio reacciona a los eventos de los demás y publica los suyos propios; la secuencia completa solo existe como la suma de esas reacciones individuales, no en un único sitio |
| Acción de compensación | Compensating Action/Transaction | Paso de una Saga que deshace el efecto de negocio de un paso anterior ya confirmado, cuando un paso posterior falla — no es un *rollback* de base de datos (cada paso ya hizo su propio commit local), sino una operación de negocio nueva que revierte el efecto |
| Orquestación | Orchestration | Forma de implementar una Saga con un componente dedicado (el orquestador) que sí conoce y dirige la secuencia completa de pasos, llamando a cada uno explícitamente y decidiendo cuándo compensar — a diferencia de la coreografía, donde ese conocimiento no vive en ningún sitio único |
| Comando | Command | Mensaje que instruye a su destinatario a ejecutar una acción concreta ("reserva este stock"), a diferencia de un evento de dominio, que solo anuncia un hecho ya consumado ("esto ya pasó") sin dirigirse a nadie en particular |
| Interfaz sellada | Sealed Interface | Java 17: una interfaz cuyas implementaciones permitidas se declaran de forma explícita y cerrada (`permits`) — el compilador conoce el conjunto completo de tipos posibles, lo que permite un `switch` exhaustivo sobre ellos sin rama `default` |
