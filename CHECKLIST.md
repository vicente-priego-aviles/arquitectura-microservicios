...
1. ~~**Segundo microservicio: Carrito/Pedidos**~~ — persistencia políglota (capítulo 6), llamada síncrona a `servicio-catalogo` vía HTTP Service Client de Spring y extracción de anotaciones Swagger a interfaz (capítulo 7). ~~**Resiliencia de esa llamada síncrona**~~ (capítulo 8): `@Retryable`/`@ConcurrencyLimit` nativos de Spring Framework 7 (`@EnableResilientMethods`) para reintentos y límite de concurrencia — sustituyen a Spring Retry. Para circuit breaker real (máquina de estados `CLOSED`/`OPEN`/`HALF_OPEN`), Spring Framework 7 no lo cubre nativamente: se usó Resilience4j directamente, no Spring Cloud Circuit Breaker (el propio proyecto se declara superseded por Spring Framework 7 y sin más desarrollo — mismo caso que OpenFeign).
