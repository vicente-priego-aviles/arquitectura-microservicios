...
- [x] Anotaciones OpenAPI extraídas a interfaz (`ProductoControllerApi`/`CategoriaControllerApi`/`PedidoControllerApi`, capítulo 7) — el `@RestController` implementa la interfaz y se queda solo con `@Override` y el cuerpo del caso de uso; springdoc-openapi resuelve las anotaciones declaradas en interfaces (FAQ oficial), y Spring MVC hace lo mismo con `@RequestMapping`/`@GetMapping`/`@PostMapping` cuando están en la interfaz en vez de la clase
