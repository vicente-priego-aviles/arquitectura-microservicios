...
- **Excepción — implementaciones alternativas del mismo escenario**: cuando dos capítulos consecutivos en la hoja de ruta resuelven el mismo problema con mecanismos que se sustituyen entre sí en vez de sumarse (p. ej. Saga coreografiada vs. Saga orquestada sobre el mismo flujo Pedido+Inventario, capítulos 13 y 14), el segundo parte del último capítulo *anterior a ambos*, no del primero de los dos — encadenarlo forzaría borrar en el segundo capítulo casi todo lo que el primero añadió, puro ruido de historial sin valor para quien lea esa rama. El capítulo con la implementación que no continúa acumulándose (aquí, la coreografía del capítulo 13) queda como una rama "hoja" de solo comparación; la cadena de capítulos siguientes continúa a partir del que sí se sigue extendiendo (aquí, la orquestación del capítulo 14).
