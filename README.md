...
Desde el capítulo 3, cada `@RestController` acumula anotaciones de OpenAPI (`@Operation`, `@ApiResponses`) junto a las de Spring MVC (`@GetMapping`, `@PostMapping`) en la misma clase. Con `servicio-catalogo` y ahora `servicio-pedidos` repitiendo el mismo patrón, esa mezcla empieza a estorbar: cuesta ver de un vistazo qué hace el método y qué es documentación para Swagger UI.

La solución es extraer todas esas anotaciones a una interfaz — `ProductoControllerApi`, `CategoriaControllerApi`, `PedidoControllerApi` — que el `@RestController` se limita a implementar. Funciona porque springdoc-openapi soporta explícitamente anotaciones declaradas en interfaces (confirmado en su FAQ oficial), y Spring MVC resuelve igual de bien los métodos anotados con `@GetMapping`/`@PostMapping` heredados de una interfaz que declarados directamente en la clase.

```java
// servicio-pedidos/.../infraestructura/adaptador/entrada/rest/PedidoControllerApi.java
...
El `@RequestMapping("/api/pedidos")` de clase también se movió a la interfaz, no solo los métodos: la documentación de Spring recomienda declarar ahí todas las anotaciones de mapeo por igual, precisamente para no repartir la misma información de enrutado entre dos sitios distintos. El resultado es un controller que solo tiene `@Override` y el cuerpo del caso de uso — toda la documentación OpenAPI queda en un único sitio, separada de la mecánica HTTP.
...
| | Archivo | Descripción funcional | Descripción del cambio |
|:---:|---|---|:---:|
| 🌱 | [`.../infraestructura/adaptador/entrada/rest/PedidoControllerApi.java`](servicio-pedidos/src/main/java/com/javacadabra/tienda/pedidos/infraestructura/adaptador/entrada/rest/PedidoControllerApi.java) | Interfaz con las anotaciones Swagger/Spring MVC de `PedidoController` | --- |
| ✏️ | [`.../infraestructura/adaptador/entrada/rest/PedidoController.java`](servicio-pedidos/src/main/java/com/javacadabra/tienda/pedidos/infraestructura/adaptador/entrada/rest/PedidoController.java) | Controller REST de Pedidos | Implementa `PedidoControllerApi`; ya no lleva anotaciones Swagger propias |
...
| 🌱 | [`.../infraestructura/adaptador/entrada/rest/ProductoControllerApi.java`](servicio-catalogo/src/main/java/com/javacadabra/tienda/catalogo/infraestructura/adaptador/entrada/rest/ProductoControllerApi.java) | Interfaz con las anotaciones Swagger/Spring MVC de `ProductoController` (`servicio-catalogo`) | --- |
| ✏️ | [`.../infraestructura/adaptador/entrada/rest/ProductoController.java`](servicio-catalogo/src/main/java/com/javacadabra/tienda/catalogo/infraestructura/adaptador/entrada/rest/ProductoController.java) | Controller REST de Productos (`servicio-catalogo`) | Implementa `ProductoControllerApi`; ya no lleva anotaciones Swagger propias |
| 🌱 | [`.../infraestructura/adaptador/entrada/rest/CategoriaControllerApi.java`](servicio-catalogo/src/main/java/com/javacadabra/tienda/catalogo/infraestructura/adaptador/entrada/rest/CategoriaControllerApi.java) | Interfaz con las anotaciones Swagger/Spring MVC de `CategoriaController` (`servicio-catalogo`) | --- |
| ✏️ | [`.../infraestructura/adaptador/entrada/rest/CategoriaController.java`](servicio-catalogo/src/main/java/com/javacadabra/tienda/catalogo/infraestructura/adaptador/entrada/rest/CategoriaController.java) | Controller REST de Categorías (`servicio-catalogo`) | Implementa `CategoriaControllerApi`; ya no lleva anotaciones Swagger propias |
